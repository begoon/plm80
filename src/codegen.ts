...
class Codegen {
    ...
    private emitCompare(op: BinOp, lhs: Expr, rhs: Expr, t: "byte" | "word", pos: Pos): void {
        ...
        switch (op) {
            ...
            case ">":
                // a > b  <=>  !(a <= b)  <=>  !(a < b || a == b)
                // simplest: swap and reuse jc as "b < a"
                throw new CodegenError(`'>' not yet supported (swap operands or use '<' with swap)`, pos);
            case "<=":
                throw new CodegenError(`'<=' not yet supported`, pos);
...
function symLabel(sym: Sym): string {
    if (sym.kind === "var") {
        if (sym.storage === "global") return dataLabel(sym.decl.name);
